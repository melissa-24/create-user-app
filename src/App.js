...
function App() {
  ...
  const handleSubmit = e => {
    ...
    if(values.name && values.bio) {
      if(!isEditing) {
      axios.post("https://jdmfn.sse.codesandbox.io/api/users", values)
        .then(res => {
        ...
        axios.put(`https://jdmfn.sse.codesandbox.io/api/users/${values.id}`, values)
          .then(res => {
  ...
  const remove = user => {
    axios.delete(`https://jdmfn.sse.codesandbox.io/api/users/${user.id}`)
      .then(res => {
  ...
  const getUsers = () => {
    axios.get("https://jdmfn.sse.codesandbox.io/api/users")
      .then(res => {
        setUsers(res.data);
      })
      .catch(err => {
